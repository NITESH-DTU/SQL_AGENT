    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="relative w-full max-w-4xl max-h-[90vh] bg-surface border border-white/[0.08] rounded-3xl overflow-hidden flex flex-col shadow-2xl">
        <div className="px-6 py-4 border-b border-white/[0.05] flex items-center justify-between bg-white/[0.02]">
          <h3 className="text-lg font-bold text-white">{initialData ? 'Edit Widget' : 'Create New Widget'}</h3>
          <button onClick={onClose} className="p-2 hover:bg-white/5 rounded-xl text-gray-400"><X size={20} /></button>
        </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full bg-black/40 border border-white/[0.06] rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-primary/50"
                />
              </div>
                  ...
                  {[
                    ...
                    <button
                      ...
                      className={`flex flex-col items-center gap-2 p-3 rounded-2xl border transition-all ${
                        formData.widget_type === t.id ? 'bg-primary/10 border-primary text-primary' : 'bg-black/20 border-white/[0.04] text-gray-400 hover:border-white/10'
                      }`}
                    >
                      {t.icon}
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex gap-2">
                    <button 
                      ...
                      className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded-lg border transition-all ${analysisMode ? 'bg-secondary/10 border-secondary text-secondary' : 'bg-white/5 border-white/10 text-gray-400 hover:text-white'}`}
                    >
                ...
                {analysisMode ? (
                  <div className="p-4 rounded-xl bg-black/40 border border-white/[0.06] space-y-4">
                    <div>
                      <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block">Select Table</label>
                      <select 
                        value={analysisConfig.table}
                        onChange={e => setAnalysisConfig({...analysisConfig, table: e.target.value})}
                        className="w-full bg-black/40 border border-white/10 rounded-lg px-2 py-2 text-xs text-white focus:outline-none"
                      >
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block">Dimension (Group)</label>
                        <select 
                      ...
                      <div>
                        <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block">Measure (Value)</label>
                        <select 
                    ...
                    {analysisConfig.measure && (
                      <div>
                        <label className="text-[9px] font-bold text-gray-400 uppercase mb-1 block">Aggregation</label>
                        <div className="flex gap-1.5">
                          {['COUNT', 'SUM', 'AVG', 'MIN', 'MAX'].map(agg => (
                            <button
                              ...
                              className={`px-2 py-1 rounded text-[10px] font-bold border transition-all ${analysisConfig.aggregation === agg ? 'bg-primary/20 border-primary text-primary' : 'bg-white/5 border-white/5 text-gray-400'}`}
                            >
                              {agg}
                            </button>
                          ))}
                  ...
                  <div className="space-y-3">
                    <div className="relative group">
                      <input 
                        ...
                        className="w-full bg-primary/5 border border-primary/20 rounded-xl pl-4 pr-12 py-2.5 text-xs text-white focus:outline-none focus:border-primary/50"
                      />
                      <button 
                        type="button"
                    ...
                    <textarea 
                      ...
                      className="w-full h-32 bg-black/40 border border-white/[0.06] rounded-xl p-4 text-[13px] font-mono text-white focus:outline-none focus:border-primary/50 resize-none scroll-thin"
                ...
                <button 
                  ...
                  className="mt-2 flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-bold text-white transition-all border border-white/10 w-full justify-center"
                >
              ...
              {formData.widget_type === 'chart' && (
                <div className="p-4 rounded-2xl bg-black/20 border border-white/[0.04] space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 block">Chart Type</label>
                      <select 
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 block">X-Axis / Label</label>
                      <select 
                    ...
                    <div>
                      <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1.5 block">Y-Axis / Value</label>
                      <select 
              ...
              <div>
                ...
                <div className="w-full h-40 bg-black/40 rounded-2xl border border-white/[0.04] flex items-center justify-center p-4">
                  {isExecuting ? (
                    ...
                    <div className="text-[10px] text-gray-400 font-mono overflow-auto max-h-full">
                      {JSON.stringify(previewData.slice(0, 3), null, 2)}
                    </div>
                  ) : (
                    <span className="text-[10px] text-gray-400 italic">Run query to see preview</span>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="px-6 py-4 border-t border-white/[0.05] flex items-center justify-end gap-3 bg-white/[0.02]">
          <button onClick={onClose} className="px-5 py-2 rounded-xl text-sm font-semibold text-gray-400 hover:bg-white/5 transition-all">Cancel</button>
